      ...
      {(props) => (
        <div className="custom-product-card">
          <div className="custom-image-container">
            <img src={props.product.itemImageUrl} alt={props.product.itemName} />
          </div>
          <div className="custom-content">
            <h3>{props.product.itemName}</h3>
            <p className="custom-price">
              {props.itemPriceCurrency}{props.product.itemPrice}
            </p>
...
function CompactListStyleCard(props) {
  return (
    <div className='flex items-center bg-white border border-gray-200 rounded-lg p-3 max-w-lg hover:bg-gray-50 transition-colors'>
      <img
        src={props.product.product.itemImageUrl}
        alt={props.product.product.itemName}
        className='w-16 h-16 object-cover rounded-lg mr-4'
      />
      <div className='flex-1'>
        <h3 className='font-semibold text-gray-800'>{props.product.product.itemName}</h3>
        <div className='flex items-center gap-2 mt-1'>
          <div className='flex items-baseline gap-1'>
            <span className='font-bold text-red-600'>
              {props.itemPriceCurrency}
              {props.product.product.itemSalePrice}
            </span>
            <span className='text-sm text-gray-400 line-through'>
              {props.itemPriceCurrency}
              {props.product.product.itemPrice}
            </span>
          </div>
          <div className='flex items-center gap-1 text-xs'>
            <span className='text-yellow-500'>⭐</span>
            <span>{props.product.product.itemRating}</span>
          </div>
        </div>
      </div>
      <button
